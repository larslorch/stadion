...
class iResBlock(nn.Module):
    ...
    def predict_from_noise(self, noise_vec, n_iter=10, intervention_set=[None], init_provided=False, x_init=None, imperfect=False):
        if init_provided:
            x = torch.tensor(x_init).float().to(noise_vec.device) 
        else:
            x = torch.randn(noise_vec.size(), device=noise_vec.device)
        c = torch.zeros_like(x)
        U = torch.eye(x.shape[1], device=x.device)
        if not imperfect:
            U[intervention_set, intervention_set] = 0
        if intervention_set and intervention_set[0] != None:
            if x_init.shape[-1] == len(intervention_set):
                c[:, intervention_set] = torch.tensor(x_init).float().to(noise_vec.device)
            else:
                c[:, intervention_set] = torch.tensor(x_init[:, intervention_set]).float().to(noise_vec.device)

        for _ in range(n_iter):
            x = self.f(x - self.mu) @ U + noise_vec @ U + c + self.mu
