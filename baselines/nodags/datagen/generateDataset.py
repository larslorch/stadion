...
from baselines.nodags.datagen.structuralModels import linearSEM, nonlinearSEM
...
class Dataset:
    ...
    def __init__(
        ...
        if graph_provided:
            ...
            generator = DirectedGraphGenerator(nodes=self.n_nodes, expected_density=self.expected_density,
        ...
        if gen_model_provided:
            ...
            if self.sem_type == 'lin':
                self.gen_model = linearSEM(self.graph, self.abs_weight_low, self.abs_weight_high,
                                           noise_scale=self.noise_scale, contractive=contractive)
            elif self.sem_type == 'nnl':
                self.gen_model = nonlinearSEM(self.graph, lip_const=self.lip_constant, noise_scale=self.noise_scale,
                                              n_hidden=self.n_hidden, act_fun=self.act_fun)

    def generate(self, rng, interventions=True, change_targets=False, fixed_interventions=False):
        dataset = list()
        ...
        if interventions:
            ...
            for target_set in self.targets:
                data = self.gen_model.generateData(rng=rng, n_samples=self.n_samples, intervention_set=target_set,
                                                   fixed_intervention=fixed_interventions)
                dataset.append(data)

        else:
            data = self.gen_model.generateData(rng=rng, n_samples=self.n_samples, intervention_set=[None])
            dataset.append(data)
...
def generate_data_learning(n_nodes_list, exp_dens_list, n_exp_list, n_samples, data_output_path, n_graphs=5,
                           return_graph=True, sem_type='lin', n_hidden=0, act_fun='tanh', lip_const=0.99,
                           mode='indiv-node', enforce_dag=False, contractive=True, interventions=True):
...
def generate_data_validate(n_nodes_list, data_output_path, graphs_list, gen_model_list, n_targets_list, n_exp=10,
                           n_samples=5000, sem_type='lin'):
    if not os.path.exists(data_output_path):
        os.makedirs(data_output_path)
