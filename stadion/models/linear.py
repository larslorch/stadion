...
def f(x, theta, intv_theta, intv):
    ...
    # [d, d] shift_per_dx_j[j] is the shift for `x` associated to the input to dx_j in the given `intv_theta`
    shift_per_dx_j = jnp.where(jnp.eye(d), jnp.diag(shift), jnp.zeros((d, d)))

    # broadcast `x` to an additional axis and then modify `x` differently for each dx_j across that axis
    # [..., d, d]
    x_per_dx_j = jnp.where(intv, x[..., None, :] - shift_per_dx_j,  x[..., None, :])

    # compute drift scalar f(x)_j for each dx_j using the input x modified correspondingly for the dx_j
    f_vec  = vmap(f_j, in_axes=(-2, 0), out_axes=-1)(x_per_dx_j, theta)

    assert x.shape[-1] == d
    assert intv.shape[0] == d and intv.ndim == 1
    assert shift_per_dx_j.shape == (d, d)
    assert x_per_dx_j.shape == (*x.shape[:-1], d, x.shape[-1])
...
def sigma(x, theta, intv_theta, intv):
    ...
    assert shift.shape == scale.shape == (d, )
    ...
    # compute diffusion vector sigma(x)_j for each dx_j using the input x modified correspondingly for the dx_j
    sig_mat = vmap(sigma_j, in_axes=(-2, 0, 0), out_axes=-2)(x_per_dx_j, theta, jnp.eye(d))
